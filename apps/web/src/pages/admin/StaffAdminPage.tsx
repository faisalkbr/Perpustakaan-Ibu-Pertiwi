...
export default function StaffAdminPage() {
  ...
  const { data, isLoading, isFetching } = useStaff({ search: debounced, page })
  const staffList = data?.data ?? []
  const meta = data?.meta

  const onDelete = (staff: Staff) => {
    if (window.confirm(`Hapus akun staf "${staff.name}"?`)) deleteStaff.mutate(staff.id)
  }

  return (
    <div className="animate-in fade-in duration-300">
      <h1 className="mb-5 font-serif text-[22px] font-semibold text-ink">Data Staf</h1>
                ...
                return (
                  <TableRow key={staff.id}>
                    <TableCell className="text-muted-foreground">
                      S-{String(100 + staff.id)}
                    </TableCell>
                    <TableCell className="font-serif font-semibold text-ink">
                      {staff.name}
                      {isSelf && (
                    ...
                    <TableCell className="text-[#55504a]">{staff.email}</TableCell>
                    <TableCell>
                      <Badge variant={staff.role === 'head' ? 'warn' : 'default'}>
                        {ROLE_LABELS[staff.role]}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant={staff.is_active ? 'ok' : 'default'}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          variant="ghost"
                          className="size-8"
                          onClick={() => setDialog({ open: true, staff })}
                          aria-label="Edit"
                        >
      ...
      {meta && (
        <div className="mt-4 flex items-center justify-between text-sm">
          <span className="text-muted-foreground">
            Menampilkan {meta.from ?? 0}–{meta.to ?? 0} dari {meta.total} staf
          </span>
      ...
      <StaffFormDialog
        open={dialog.open}
        staff={dialog.staff}
        isSelf={dialog.staff?.id === currentUser?.id}
